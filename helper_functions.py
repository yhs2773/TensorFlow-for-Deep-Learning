...
def load_and_prep_image(filename, img_shape=224, scale=True):
    ...
    and reshapes into (224, 224, 3).

    Parameters
    ----------
    filename (str): string filename of target image
    img_shape (int): size to resize target image to, default 224
    scale (bool): whether to sccale pixel values to range(0, 1), default True
    """
    # Read in the image
...
def make_confusion_matrix(y_true, y_pred, classes=None, figsize=(10, 10), text_size=15, norm=False, savefig=False):
    ...
    Args:
        ...
        norm: Whether to normalize values or not (default = False).
...
def pred_and_plot(model, filename, class_names):
    ...
    makes a prediction on it with a trained model,
...
def compare_histories(original_history, new_history, initial_epochs=5):
    ...
    # Combine original history with the new history
